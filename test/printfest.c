...
int	main(void)
{
	...
	ret = dprintf(1, "%c %c %c %c %c %c", 'X', 0, 42, -42, INT_MIN, INT_MAX); RET;
	dprintf(1, "\n  ---STR---\n\n");
	ret = ft_printf ("%s %s %s", "string", "", null); RET;
	ret = dprintf(1, "%s %s %s", "string", "", null); RET;
	dprintf(1, "\n  ---NBR---\n\n");
	ret = ft_printf ("%i %i %i %i %i %i", 'X', 0, 42, -42, INT_MIN, INT_MAX); RET;
	ret = dprintf(1, "%i %i %i %i %i %i", 'X', 0, 42, -42, INT_MIN, INT_MAX); RET;
	dprintf(1, "\n  ---UNS---\n\n");
	ret = ft_printf ("%u %u %u %u %u %u", 'X', 0, 42, -42, INT_MIN, INT_MAX); RET;
	ret = dprintf(1, "%u %u %u %u %u %u", 'X', 0, 42, -42, INT_MIN, INT_MAX); RET;
	dprintf(1, "\n  ---HX1---\n\n");
	ret = ft_printf ("%x %x %x %x %x %x", 'X', 0, 42, -42, INT_MIN, INT_MAX); RET;
	ret = dprintf(1, "%x %x %x %x %x %x", 'X', 0, 42, -42, INT_MIN, INT_MAX); RET;
	dprintf(1, "\n  ---HX2---\n\n");
	ret = ft_printf ("%X %X %X %X %X %X", 'X', 0, 42, -42, INT_MIN, INT_MAX); RET;
	ret = dprintf(1, "%X %X %X %X %X %X", 'X', 0, 42, -42, INT_MIN, INT_MAX); RET;
	dprintf(1, "\n  ---HX2---\n\n");
	...
	ret = dprintf(1, "%p %p %p", (void *)null, (void *)&ret, NULL); RET;
	dprintf(1, "\n  ---MIX---\n\n");
	ret = ft_printf ("- %% %c %s %i %u %x %X %x %p ", 97, "b", 0, -1, -2, -3, -4, (void *)null); RET;
	ret = dprintf(1, "- %% %c %s %i %u %x %X %x %p ", 97, "b", 0, -1, -2, -3, -4, (void *)null); RET;
	return (0);
}
